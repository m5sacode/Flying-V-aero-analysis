...
def parse_file(filename, inverty=False):
    ...
    data_started = False
    values = []
    expected_vars = 4  # X, Y, Z, Static Pressure
    node_count = None

    for line in lines:
        line = line.strip()

        # Parse node count from line like "I=223, J=1, K=1"
        if line.startswith("I="):
            match = re.search(r"I=(\d+)", line)
            if match:
                node_count = int(match.group(1))

        # Begin parsing block data after DT= line
        elif line.startswith("DT="):
            data_started = True
            values = [[] for _ in range(expected_vars)]
            current_var = 0
            current_list = []
            count = 0
            continue
        ...
        if data_started:
            for num in line.split():
                try:
                    ...
                    count += 1
                    if count == node_count:
                        values[current_var] = current_list
                        current_list = []
                        count = 0
                        current_var += 1
                        if current_var == expected_vars:
                            data_started = False
                            break
                except ValueError:
                    pass  # Skip anything that's not a number

    if len(values) != expected_vars or any(len(v) != node_count for v in values):
        raise Exception("Parsed data does not match expected format")

    Xs, Ys, Zs, Statics = values
    if inverty:
        Ys = [-y for y in Ys]
...
def compute_lift_coefficient(x, y, z, pressure, q, S):
    ...
    """
    mask = (np.abs(y) <= 1.5) & (np.abs(pressure) > 1)
    x_filtered = x[mask]
    y_filtered = y[mask]
    z_filtered = z[mask]
